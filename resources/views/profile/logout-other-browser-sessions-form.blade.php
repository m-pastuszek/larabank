<x-jet-action-section>
    <x-slot name="title">
        {{ __('pages.profile.sessions') }}
    </x-slot>

    <x-slot name="description">
        {{ __('pages.profile.sessions-info') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('pages.profile.sessions-desc') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('pages.profile.this-device') }}</span>
                                    @else
                                        {{ __('pages.profile.last-active') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('pages.profile.logout-browsers') }}
            </x-jet-button>

            <x-jet-action-message class="ml-3" on="loggedOut">
                {{ __('pages.profile.done') }}
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('pages.profile.password-confirmation') }}
